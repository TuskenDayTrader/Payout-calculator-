...
test("desktop smoke test", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: /Eligibility Results/i })).toBeVisible();
  await page.getByLabel(/Current balance/i).fill("53400");
  await expect(page.getByText(/Safe max request/i).first()).toBeVisible();
});
...
test("mobile smoke test", async ({ page }) => {
  ...
  await expect(page.getByLabel(/Current payout #/i)).toBeVisible();
});
